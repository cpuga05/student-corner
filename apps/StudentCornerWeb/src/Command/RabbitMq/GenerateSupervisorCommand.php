...
use Shared\Infrastructure\Bus\Event\DomainEventConsumerLocator;
...
final class GenerateSupervisorCommand extends Command
{
    protected static $defaultName = 'rmq:supervisor';
    ...
    public function __construct(DomainEventConsumerLocator $locator)
    {
        parent::__construct();
    ...
    {
        return function (DomainEventConsumer $subscriber) use ($path) {
    ...
    private function template(): string
    {
        return <<<EOF
[program:{queue_name}]
command      = {path}/apps/StudentCornerWeb/bin/console event:consume --env=prod {queue_name} {events_to_process}
